...
export interface PlayerRoundAnalysis {
  ...
  samples: Array<{ frame: number; x: number; y: number; z: number; speed: number; yaw: number; pitch: number; health: number; armor: number; weapon: string; radar?: { x: number; y: number }; callout?: string }>;
