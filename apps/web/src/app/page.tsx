...
interface DeepTaskResponse {
  ...
  progress: number | object;
  failedReason?: string;
  result?: { analysis: { summary: { shotsFired: number; damageDealt: number; damageTaken: number; movingShots: number }; initialState?: { frame: number; health: number; armor: number; money: number; equipmentValue: number; weapon: string }; freezeTimeEndFrame?: number; roundTimeSeconds?: number; samples: Array<{ frame: number; x: number; y: number; z: number; yaw: number; pitch: number; speed: number; health: number; armor: number; weapon: string; radar?: { x: number; y: number }; callout?: string }>; otherPlayerSamples?: Array<{ frame: number; steamId64: string; name: string; team: string; x: number; y: number; z: number; health: number; radar?: { x: number; y: number } }>; events: Array<{ frame: number; type: string; opponent: string; weapon: string; damage: number; speed: number; stopStatus?: string; confidence?: string }> } };
}
...
function RadarEvidenceCard({ frame, samples, events, tickRate, otherPlayerSamples = [], showOtherPlayers = false }: { frame: number; samples: NonNullable<DeepTaskResponse["result"]>["analysis"]["samples"]; events: NonNullable<DeepTaskResponse["result"]>["analysis"]["events"]; tickRate: number; otherPlayerSamples?: NonNullable<DeepTaskResponse["result"]>["analysis"]["otherPlayerSamples"]; showOtherPlayers?: boolean }) {
  const points = samples.filter((sample): sample is typeof sample & { radar: { x: number; y: number } } => sample.radar !== undefined);
  ...
  const otherPlayersAtFrame = showOtherPlayers ? [...new Map(otherPlayerSamples.filter((sample): sample is typeof sample & { radar: { x: number; y: number } } => sample.radar !== undefined && Math.abs(sample.frame - selected.frame) <= 1).sort((a, b) => Math.abs(a.frame - selected.frame) - Math.abs(b.frame - selected.frame)).map((sample) => [sample.steamId64, sample])).values()] : [];
  ...
  return <article className="radar-evidence-card">
    ...
    <div className="radar-evidence-map"><img src="/maps/de_mirage/radar.png" alt={`帧 ${selected.frame} 的 Mirage 雷达位置`} /><svg viewBox="0 0 1024 1024" aria-label="AI 引用位置"><polyline points={path} className="radar-evidence-path" />{nearbyEvents.map(({ event, sample }, index) => <circle key={`${event.frame}-${index}`} cx={sample.radar.x} cy={sample.radar.y} r="7" className={`radar-event radar-event-${event.type}`}><title>帧 {event.frame} · {deepEventText(event)}</title></circle>)}{otherPlayersAtFrame.map((player) => <circle key={player.steamId64} cx={player.radar.x} cy={player.radar.y} r="9" className={player.team === "T" ? "radar-other-player radar-other-player-t" : "radar-other-player radar-other-player-ct"}><title>{player.name} · {player.team} · {player.health} HP</title></circle>)}<line x1={selected.radar.x} y1={selected.radar.y} x2={arrowX} y2={arrowY} className="radar-evidence-heading-arrow" /><circle cx={selected.radar.x} cy={selected.radar.y} r="13" className="radar-evidence-position" /></svg></div>
    ...
    <p className="hint">展示该帧前后约 3 秒二维轨迹；箭头为玩家平面朝向，点位名称仅作图片区域近似。{showOtherPlayers ? " 红点为 T、蓝点为 CT 的其他存活玩家。" : ""}</p>
  </article>;
}
